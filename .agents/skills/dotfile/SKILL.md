...
The whole system is built around one idea: **the repo only holds shared config; every machine's secrets, paths, and personal settings stay out of it.** A handful of files are split in two by **markers** — everything above the marker is repo-managed, everything below is machine-local and must never be committed.
...
When you're inside the dotfiles repository as a git checkout — e.g. `/home/ye/code/dotfile` — you're authoring the repo's own *content*: config files, README/docs, `.agents/`, `.skill-lock.json`, these skill files. Use **plain `git`** here (`git status`, `git add <file>`, `git commit`, `git push origin main`, review with `git diff --cached`). The repo's checked-out files *are* the managed content (no local-only sections), so there are **no markers to respect and no `/tmp` isolation needed**. The only rule that still applies: never commit secrets.

### Operating an installed machine's config (the **production** environment)
When you're on a machine whose dotfiles are deployed as a bare repo at `$HOME/.cfg` with work-tree `$HOME`, you're touching the LIVE configuration (`~/.zshrc`, `~/.config/...`). Use **`dgit`** here. This is where markers, the `/tmp` isolated-clone push, the `LOCAL [never push]` commit, and the migration / `dgit pull` workflows all live.

> **Everything in this skill's references (`install`, `maintain`, `update`, `migrations`, `conventions`) describes the production/machine path (`dgit`) — not the source repo.** If you are in the source repo, just use normal `git`; none of the marker,/tmp/LOCAL machinery applies.
...
- **Marker files split managed vs local.** Above the marker = repo-managed (remote wins on conflict). Below the marker = machine-local (preserve, never commit). The three files with markers are `.zshrc`, `.codex/config.toml`, and `.config/opencode/opencode.jsonc`. Full details in [references/conventions.md](references/conventions.md).
...
- **Pushing shared changes off a machine that carries local content must happen in an isolated clone under `/tmp`**, stripping the local sections first. Pushing from the work-tree can leak local config into the public repo.
- **Never commit** private keys, tokens, `.proxyenv`, `.zprofile`, `.ssh/config`, `.npmrc`, `.gitconfig`, or anything below a local marker.
